    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 每 flushPhysicQueueThoroughInterval 周期，执行一次 flush 。
                // 因为不是每次循环到都能满足 flushCommitLogLeastPages 大小，因此，需要一定周期进行一次强制 flush 。
                ...
                try {
                    // 等待执行
                    // 根据 flushCommitLogTimed 参数，可以选择每次循环是固定周期还是等待唤醒。默认配置是后者，
                    // 所以，每次插入消息完成，会去调用 commitLogService.wakeup()
            ...
            synchronized (this.requestsRead) {
                if (!this.requestsRead.isEmpty()) {
                    for (GroupCommitRequest req : this.requestsRead) {
                        ...
                        // 考虑到有可能每次循环的消息写入的消息，可能分布在两个 MappedFile(写第N个消息时，MappedFile 已满，创建了一个新的)，
                        // 所以需要有循环2次
